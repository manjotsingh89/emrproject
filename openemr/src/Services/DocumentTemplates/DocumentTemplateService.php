...
// can delete
    ...
    public function renderPortalTemplateMenu($current_patient, $current_user, $dropdown = false): string
    {
        ...
        ksort($all_templates);
        //echo '<pre>'; print_r($all_templates); echo '</pre>';
        foreach ($all_templates as $category => $templates) {
            if (is_array($templates)) {
                ...
                foreach ($templates as $template) {
                    ...
                    //echo '<pre>';print_r($template['pid']);echo '</pre>';
                    if($btnname == 'Insurance Info'){
                        $actual_link = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]";
                        $webroot = $GLOBALS['web_root'];
                        $linkd = $actual_link.$webroot.'/portal/patient/onsitedocumentsdemo?pid='.$template['pid'].'&questionnaire=1';
                        $link1 = $actual_link.$webroot.'/portal/patient/onsitedocuments?pid='.$template['pid'].'&questionnaire=1';
                        $link2 = $actual_link.$webroot.'/portal/patient/onsitedocumentstwo?pid='.$template['pid'].'&questionnaire=1';
                        $link3 = $actual_link.$webroot.'/portal/patient/onsitedocumentsthree?pid='.$template['pid'].'&questionnaire=1';
                        
                        $menu .= '<a class="dropdown-item template-item text-success btn btn-link" id=""' . ' href="'.$linkd.'" target="_blank">' . 'Demographics' . "</a>\n";
                        $menu .= '<a class="dropdown-item template-item text-success btn btn-link" id=""' . ' href="'.$link1.'" target="_blank">' . 'Questionnaire1' . "</a>\n";
                        //$menu .= '<a class="dropdown-item template-item text-success btn btn-link" id=""' . ' href="https://emr.livebroadcast.co.za/openemr/portal/patient/onsitedocuments?pid=2&questionnaire=1" target="_blank">' . 'Questionnaire1' . "</a>\n";
                        $menu .= '<a class="dropdown-item template-item text-success btn btn-link" id=""' . ' href="'.$link2.'" target="_blank">' . 'Questionnaire2' . "</a>\n";
                        $menu .= '<a class="dropdown-item template-item text-success btn btn-link" id=""' . ' href="'.$link3.'" target="_blank">' . 'Questionnaire3' . "</a>\n";
                    }
                }
                if (!$flag) {
